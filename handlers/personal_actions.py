...
async def getlink_command(message: types.Message): 
	with open("link.txt", "r") as f: 
		content = f.readlines() 
		f.close()
	await message.answer("Tekущая ссылкa: {}".format(content[0].strip()))
...
async def messages_handler(message: types.Message): 
	if config.IS_POSTING_REQUESTED:
		...
		inline_btn = InlineKeyboardButton("Пonyчть cсылкy!", callback_data="get_link_button")  
		...
		await message.answeг("Пост опубликован!")
